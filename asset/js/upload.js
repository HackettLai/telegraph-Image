...
$smfile.fileinput({
  ...
  // uploadLabel: "上传",
  dropZoneTitle:
    "Drag and drop files here<br>Or copy and paste the screenshot here<br>Support multiple files uploading at the same time...",
  //showPreview: false
});
...
function render_uploaded() {
  ...
  uploaded_files.forEach(function (x) {
    ...
    var url =
      resp.data.thumb == null
        ? BASE_PROXYURL + resp.data.url
        : BASE_PROXYURL + resp.data.thumb.url;
    var wpurl =
      resp.data.thumb == null
        ? WPRAW + resp.data.url
        : WPRAW + resp.data.thumb.url;
    var RAW =
      resp.data.thumb == null
        ? PhRAW + resp.data.url
        : PhRAW + resp.data.thumb.url;
...
$.prototype.fileinput.Constructor.prototype["setBeforeAjaxEnqueue"] = function (
  func
) {
  this.beforeAjaxEnqueue = func;
};

var upload_baseurl = `${$smfile.fileinput(
  "getUploadUrl"
)}?ts=${Date.now()}&rand=${Math.random()}`;

$smfile.on("filelock", function (event, filestack, extraData) {
  $smfile.fileinput(
    "setUploadUrl",
    `${upload_baseurl}&batch_size=${Object.keys(filestack).length}`
  );
});
...
var clipboard = new ClipboardJS("#copyAllButton", {
  text: function () {
    var cpurls = Array.from(
      document.querySelectorAll(`#${currentTab} .cpurl`)
    ).map(function (input) {
      return input.value;
    });
